    ...
    #    if next not in d:
     #       break
      #  wordlist.append(next)
       # next = random.choice(d[prefixes])
    #return " ".join(wordlist)


def generate_text(d,start_word,length=50):
    wordlist = []
    prefixes = start_words
    for i in range(length):
        if prefixes not in d:
            break
        wordlist.append(prefixes)
    for i in range(length):
        if prefixes not in d:
            break
        wordlist.append(prefixes)   
        prefixes = random.choice(d[prefixes])
    return " ".join(wordlist)

"""
I tried appending the next word to the
prefixes variable, but I am having trouble
with it"""
